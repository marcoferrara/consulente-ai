...
def main():
    zip_filename = "social_pesca_deploy.zip"
    ...
    exclude_files = {
        ...
        'index.html', 'social_pesca_hero.png'
    }
    
    print("=" * 60)
    print("  CREATORE DI PACCHETTI DI DEPLOY — SOCIAL PESCA")
    print("=" * 60)
    ...
    try:
        ...
        print("-" * 60)
        print("COMPLETATO CON SUCCESSO! [OK]")
        print(f"Creato archivio:       {zip_filename}")
        print(f"File inseriti:         {count}")
        print(f"Dimensione archivio:   {zip_size:.2f} MB")
        print("-" * 60)
        print("Istruzioni:")
        print("1. Condividi il file 'social_pesca_deploy.zip' con il cliente o caricalo sul server.")
        print("2. Scompatta lo zip nella directory di esecuzione sul server.")
        print("3. Configura il file .env e avvia il server (vedi app.py).")
        print("=" * 60)
