...
async def analyze_feasibility(req: FeasibilityRequest):
    ...
    Controlli di compatibilità cumulativi (S.r.l.s. con Unico Socio Lavoratore):
    ...
    prompt = f"""
    Sei il consulente senior per la finanza agevolata e i bandi di 'Consulente AI'. Devi redigere una perizia di fattibilità tecnica e legale incrociando i dati di un bando d'impresa con il profilo del cliente '{profile.get('name', 'Social Pesca')}' di {profile.get('headquarters', 'Oristano')} (S.r.l.s. con unico socio lavoratore) e considerando i vincoli cumulativi di gestione del singolo lavoratore.
    ...
    Compila un'analisi di fattibilità estremamente rigorosa per microimprese. Integra i controlli cumulativi:
    ...
    {{
      ...
      "legal_analysis": "Analisi approfondita su ATECO, forma societaria S.r.l.s., regole De Minimis e idoneità formale del bando per imprese commerciali.",
      "technical_analysis": "Valutazione sulla capacità operativa dell'unico socio lavoratore (sovraccarichi di ore per la gestione della pratica e l'esecuzione del progetto).",
...
async def generate_project_draft(req: FeasibilityRequest):
    ...
    prompt = f"""
    Sei il capo del team di progettazione d'impresa per 'Consulente AI'. Devi redigere una proposta di progetto/Business Plan per candidare la società '{profile.get('name', 'Social Pesca')}' (S.r.l.s. a socio unico) al bando indicato.
    ...
    Nel campo 'academic_path_advice' (che interpreteremo come consigli commerciali), descrivi una roadmap commerciale per consolidare accordi di co-marketing con guide di pesca, noleggi barche ed e-commerce locale nel Sinis.
    
    Devi allineare e personalizzare rigorosamente la proposta di progetto, i costi stimati nel budget e le azioni principali alle priorità e obiettivi specificati dal cliente (se fornite), personalizzando il titolo, il sommario, le macro-azioni e la roadmap in modo che riflettano la sua reale richiesta.

    Per ciascuna delle 4 chiavi di 'post_award_roadmap' (accettazione_e_avvio, esecuzione_investimenti, documentazione_rendicontazione, pratiche_e_collaudo) inserisci all'interno del testo descrittivo del bando un riferimento preciso all'articolo del bando ed un link HTML cliccabile funzionante che punti all'URL ufficiale del bando '{grant.get('official_link')}' formattato ESATTAMENTE come: <a href='{grant.get('official_link')}' target='_blank' style='color: var(--primary); font-weight: 600; text-decoration: underline;'>[Rif. Bando: Art. X]</a>.

    RISPONDI ESATTAMENTE con questo oggetto JSON (senza testo di contorno):
    {{
      "grant_id": "{grant.get('id')}",
      "project_title": "Titolo originale e ad alto impatto per il progetto commerciale",
      "project_summary": "Sintesi strategica del progetto di digitalizzazione ed efficientamento del negozio da presentare (max 4 frasi). Includi l'integrazione di distributore esche H24 e e-commerce.",
      "key_actions": [
        "Azione 1: Ammodernamento ed allestimento vetrine ed acquisizione distributore refrigerato H24",
        "Azione 2: Sviluppo della piattaforma e-commerce e catalogo WhatsApp Business per il conversational commerce",
        "Azione 3: Installazione impianto solare fotovoltaico per l'abbattimento dei costi delle celle frigorifere"
      ],
      "budget_draft": {{
        ...
        "costi_attrezzature_tecnologiche": "Acquisto distributore automatico H24, kit e-commerce e fotovoltaico (es. 28000 €)",
      ...
      "partnership_strategy": "Descrizione strategica su quali aziende e operatori turistici del Sinis coinvolgere in accordi di co-marketing per amplificare i flussi di acquisto (noleggi barche, guide pesca, skipper, hotel locali).",
      "academic_path_advice": "Roadmap commerciale nel Sinis: 1. Siglare un protocollo con charter e skipper di pesca di Torregrande per consigliare Social Pesca per l'acquisto di esche vive ed esche artificiali Shimano prima della partenza. 2. Integrare un QR code sul distributore esterno per consentire l'acquisto rapido tramite WhatsApp di kit specifici.",
      "post_award_roadmap": {{
        "accettazione_e_avvio": "Firmare digitalmente l'Atto di Sottomissione e trasmetterlo all'ente erogatore tramite PEC entro 30 giorni dalla pubblicazione della graduatoria, allegando eventuale richiesta di anticipo del 35% assistita da fideiussione bancaria o assicurativa. <a href='{grant.get('official_link')}' target='_blank' style='color: var(--primary); font-weight: 600; text-decoration: underline;'>[Rif. Bando: Art. 10 - Adesione e Avvio]</a>",
        "esecuzione_investimenti": "Effettuare l'ordine formale delle attrezzature ed eseguire i lavori entro 6 mesi. N.B.: Tutti i pagamenti dei preventivi approvati (es. distributore automatico H24, fotovoltaico) devono essere effettuati tramite bonifico bancario parlante tracciabile indicando obbligatoriamente il codice CUP (Codice Unico Progetto) della misura per evitare l'inammissibilità della spesa. <a href='{grant.get('official_link')}' target='_blank' style='color: var(--primary); font-weight: 600; text-decoration: underline;'>[Rif. Bando: Art. 12 - Tracciabilità delle Spese]</a>",
        "documentazione_rendicontazione": "Fascicolo finale da trasmettere entro 9 mesi: fatture elettroniche quietanzate inserite a SDI, estratti conto completi dimostrativi della valuta, dichiarazione liberatoria dei fornitori attestante il saldo delle fatture, e DURC aziendale in corso di validità. <a href='{grant.get('official_link')}' target='_blank' style='color: var(--primary); font-weight: 600; text-decoration: underline;'>[Rif. Bando: Art. 15 - Rendicontazione e Saldo]</a>",
        "pratiche_e_collaudo": "Per la vendita H24 presentare la SCIA commerciale al SUAPE del Comune di Oristano. Allegare certificati di conformità degli impianti del fotovoltaico rilasciati dall'installatore e perizia asseverata finale a firma del commercialista/revisore contabile per la validazione delle spese. <a href='{grant.get('official_link')}' target='_blank' style='color: var(--primary); font-weight: 600; text-decoration: underline;'>[Rif. Bando: Art. 16 - Controlli e SCIA]</a>"
      }}
    }}
    """
    ...
    def get_offline_project_draft():
        ...
        return {
            ...
            "project_summary": f"Il progetto si propone di rivoluzionare il modello operativo di Social Pesca a Oristano, integrando un distributore refrigerato automatico intelligente H24 per esche vive e minuteria, sviluppando un portale e-commerce interattivo per la vendita di attrezzature, e installando un micro-impianto fotovoltaico per ridurre l'impronta energetica.{summary_suffix}",
            "key_actions": [
              "Fase 1: Acquisizione ed installazione esterna del distributore refrigerato automatico H24 ad alta efficienza energetica.",
              ...
              "Fase 3: Transizione energetica tramite installazione di impianto fotovoltaico per coprire i consumi fissi di celle refrigerate."
            ],
            "budget_draft": {
              "costi_personale": "1,500 € (Consulenti esterni)",
              "costi_viaggio_mobilita": "1,000 € (Spese di spedizione e montaggio)",
              "costi_attrezzature_tecnologiche": f"{round(grant.get('budget_max', 40000) * 0.78):,} € (Beni strumentali)",
              "costi_consulenze_esterne": f"{round(grant.get('budget_max', 40000) * 0.12):,} € (Commercialista, Agenti web)",
              "totale_stimato": f"{round(grant.get('budget_max', 40000) * 0.95):,} €"
            },
            "checklist_documents": [
              ...
              "Contratto di affitto del locale commerciale di Via della Conciliazione"
            ],
            "external_professionals": [
              "Consulente Finanza Agevolata per gestione prima bando",
            ...
            "post_award_roadmap": {
                ...
                "esecuzione_investimenti": f"Effettuare l'ordine formale delle attrezzature ed eseguire i lavori entro 6 mesi. N.B.: Tutti i pagamenti dei preventivi approvati (es. distributore automatico H24, fotovoltaico) devono essere effettuati tramite bonifico bancario parlante tracciabile indicando obbligatoriamente il codice CUP (Codice Unico Progetto) della misura per evitare l'inammissibilità della spesa.{link_esecuzione}",
...
async def compile_grant_application(req: FeasibilityRequest):
    ...
    I testi generati per ogni sezione (A-H) DEVONO essere strettamente allineati e coerenti con le priorità e gli obiettivi principali indicati dal cliente, personalizzando le descrizioni tecnologiche, il piano delle attività, la giustificazione della spesa e il ROI in base a quanto richiesto.
    ...
    {{
      ...
      "sezione_b_descrizione_progetto": "Testo (6-8 righe) per descrivere il progetto commerciale. Includi un titolo accattivante e mostra come l'investimento (distributore refrigerato H24, e-commerce, impianto fotovoltaico) risolverà un problema di produttività ed efficienza d'impresa.",
      ...
      "sezione_d_metodologia": "Testo (6-8 righe) per 'Soluzione Tecnologica e Piano Operativo'. Descrivi le specifiche del distributore H24 integrato a microprocessore, l'architettura dell'e-commerce collegata a WhatsApp Business API, e i vantaggi dell'installazione del fotovoltaico.",
      ...
      "sezione_f_budget_narrativo": "Testo (4-5 righe) per 'Giustificazione del budget'. Mostra come i costi di attrezzature 4.0, distributore automatico, fotovoltaico e consulenze web/tecniche siano congrui rispetto ai prezzi di mercato e rispettino i vincoli del bando.",
    ...
    return {
        ...
        "sezione_b_descrizione_progetto": (
            f"Il progetto strategico 'PESCA INNOVAZIONE 5.0' risponde al bando '{grant_title}' promosso da {grant_issuer}. L'intervento si propone di modernizzare ed efficientare il modello d'impresa attraverso tre pilastri tecnologici: (1) l'installazione esterna di un distributore refrigerato automatico H24 per esche vive ed accessori, garantendo vendite continuative h24/7; (2) lo sviluppo di un portale e-commerce evoluto per la prenotazione e acquisto di kit pronti per il Sinis; (3) l'acquisizione di un impianto fotovoltaico per azzerare l'impatto energetico e i costi fissi di climatizzazione delle esche.{priorities_text}"
        ),
        ...
        "sezione_d_metodologia": (
            f"La soluzione tecnologica prevede l'installazione di un distributore refrigerato ad alta tecnologia con controllo remoto di temperatura e stock tramite microprocessore. La piattaforma e-commerce sarà strutturata su architettura cloud ad alta velocità, integrando un catalogo interattivo collegato direttamente al canale WhatsApp Business API per facilitare il conversational commerce ed ottimizzare l'assistenza. L'impianto solare fotovoltaico da 4.5 kW sarà dimensionato per coprire interamente l'assorbimento delle celle frigorifere di stoccaggio delle esche."
        ),
        "sezione_e_piano_attivita": (
            f"Il cronoprogramma è pianificato in un arco temporale di 8 mesi suddiviso in tre fasi operative: "
            f"Fase 1 (Mesi 1-2): Richiesta permessi comunali, allestimento impianto elettrico ed installazione del distributore refrigerato H24. "
            ...
            f"Fase 3 (Mesi 6-8): Installazione e collaudo dell'impianto fotovoltaico di Via della Conciliazione, lancio campagne promozionali locali e rendicontazione."
        ),
        "sezione_f_budget_narrativo": (
            f"Il piano finanziario prevede un investimento totale stimato di {round(grant_budget * 0.95):,.0f} € (di cui {grant.get('financing_percentage')}% richiesto a fondo perduto). Le spese sono allocate esclusivamente in beni strumentali ad alta tecnologia: distributore refrigerato automatico ({round(grant_budget * 0.45):,.0f} €), impianto fotovoltaico d'impresa ({round(grant_budget * 0.30):,.0f} €), piattaforma software e-commerce e connettività ({round(grant_budget * 0.15):,.0f} €) e spese di consulenza/progettazione ({round(grant_budget * 0.05):,.0f} €). Le voci sono supportate da preventivi formali e sono congrue con i parametri del mercato."
        ),
        "sezione_g_impatto": (
            f"L'implementazione delle tecnologie 5.0 genererà un impatto eccezionale sulla sostenibilità e produttività aziendale. Si prevede un incremento del fatturato complessivo di almeno il 25% entro 18 mesi dall'avvio, derivante dalle vendite automatiche passive h24 e dall'e-commerce locale. L'autoproduzione energetica solare ridurrà l'impronta di carbonio della ditta ed eliminerà il 35% dei costi fissi delle celle frigorifere commerciali. Il ritorno dell'investimento (ROI) della quota di cofinanziamento di {cofi_amt:,.0f} € è stimato in soli 14 mesi."
        ),
